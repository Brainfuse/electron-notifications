...
class NotificationView {
  ...
  decorateClasses () {
    ...
    var msgLength = this.options.message.length;
    var messageContainer = document.getElementsByClassName('message')[0];

    if (msgLength >= 64) {
      //3 lines or more 
      messageContainer.classList.add('threeline');
    } else if (msgLength > 32) {
      //two lines,  title padding 10
      messageContainer.classList.add('twoline');
    } else {
      //one line default
    }
  }
